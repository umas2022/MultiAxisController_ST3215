...
def run_with_csv(csv_file, output_file):
    ...
    # 参数设置
    # 示例csv行数24000，采用频率100Hz，总时间24000/100=240s；动作频率0.05，动作步数12，总时间12/0.05=240s
    # 降采样2，实际采用点数12000，采集频率50Hz，总时间12000/50=240秒；动作频率12/240=0.05Hz
    record_frequency = 50  # 采集频率（Hz）
    downsample_csv = 2  # csv降采样因子(每n行采样一次)
    total_samples = 240  # 总采样点数
...
for i, csv_file in enumerate(csv_files, start=1):
    ...
    print(f"[{i}/{total_files}] Processing {csv_file.name}...", end="", flush=True)
