...
class sts(protocol_packet_handler):
    ...
    def ReadLoad(self, sts_id):
        """
        补充：读取负载
        参考c代码，方向由第10位决定，sts_tohost函数的第二个参数决定负载的单位
        """
    ...
    def ReadTemprature(self, sts_id):
        """
        补充：读取温度
        """
        sts_present_temper, sts_comm_result, sts_error = self.read2ByteTxRx(sts_id, STS_PRESENT_TEMPERATURE)
        return sts_present_temper, sts_comm_result, sts_error
